...
@interface XHZGJobViewController ()<UIScrollViewDelegate,CDRTranslucentSideBarDelegate,UITableViewDataSource, UITableViewDelegate>
...
@implementation XHZGJobViewController
extern XHZGJobViewController *mZgJobVc;
XHYJMainViewController *mYjMainVc;
...
- (void)setupChildVces
{
    XHDZPViewController *DJDVC = [[XHDZPViewController alloc] init];
    DJDVC.title = @"待指派";
    [self addChildViewController:DJDVC];
    
    XHDFHViewController *YJDVC = [[XHDFHViewController alloc] init];
    YJDVC.title = @"待复核";
    [self addChildViewController:YJDVC];
    
    XHZGYGQViewController *YGQVC = [[XHZGYGQViewController alloc] init];
    YGQVC.title = @"已挂起";
...
{
    // 监听待指派数量改变
    [XHNotificationCenter addObserver:self selector:@selector(DZPOrderCountDidChange:) name:XHDZPOrderCountDidChangeNotification object:nil];
    // 监听待复核数量改变
    [XHNotificationCenter addObserver:self selector:@selector(DFHOrderCountDidChange:) name:XHDFHOrderCountDidChangeNotification object:nil];
    // 监听已挂起数量改变
    [XHNotificationCenter addObserver:self selector:@selector(ZGYGQOrderCountDidChange:) name:XHZGYGQOrderCountDidChangeNotification object:nil];
    
}

- (void)DZPOrderCountDidChange:(NSNotification *)notification
{
    XYTitleButton *button = self.buttons[0];
    NSString *count = notification.userInfo[XHDZPOrderCount];
    button.countStr = count;
    NSLog(@"待接单数量%@",notification.userInfo[XHDZPOrderCount]);
}
- (void)DFHOrderCountDidChange:(NSNotification *)notification
{
    XYTitleButton *button = self.buttons[1];
    button.countStr = notification.userInfo[XHDFHOrderCount];
    NSLog(@"已接单数量%@",notification.userInfo[XHDFHOrderCount]);
}
- (void)ZGYGQOrderCountDidChange:(NSNotification *)notification
{
    XYTitleButton *button = self.buttons[2];
    button.countStr = notification.userInfo[XHZGYGQOrderCount];
    NSLog(@"已挂起数量%@",notification.userInfo[XHZGYGQOrderCount]);
}
