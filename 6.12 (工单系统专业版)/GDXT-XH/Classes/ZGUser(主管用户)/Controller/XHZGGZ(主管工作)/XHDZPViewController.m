...
//#define  selectedYJDOrderFrame self.DjdOrderFrames[self.dzpTableView.indexPathForSelectedRow.row]
static NSString *ID=@"xhdzpCell";

@interface XHDZPViewController ()<UITableViewDelegate,UITableViewDataSource,XHOrderCellDelegate>
@property (strong, nonatomic) IBOutlet UITableView *dzpTableView;
...
@implementation XHDZPViewController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self.dzpTableView registerNib:[UINib nibWithNibName:@"XHOrderCell" bundle:nil] forCellReuseIdentifier:ID];
    self.dzpTableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingTarget:self refreshingAction:@selector(loadNewData)];
    self.dzpTableView.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingTarget:self refreshingAction:@selector(loadMoreData)];
    [self.dzpTableView.mj_header beginRefreshing];
...
- (void)loadDjdOrders
{
    ...
    [XHHttpTool get:ZGDZPOrderUrl params:params success:^(NSDictionary *json) {
        ...
        [XHNotificationCenter postNotificationName:XHDZPOrderCountDidChangeNotification object:nil userInfo:@{XHDZPOrderCount: [NSString stringWithFormat:@"%lu",self.totalCount]}];
        ...
        [self.dzpTableView.mj_header endRefreshing];
        [self.dzpTableView.mj_footer endRefreshing];
        [self.dzpTableView reloadData];
...
- (void)DJDTableViewCell:(XHOrderCell *)cell didClickButton:(XHDJDTableViewButton)button {
    if (button == XHDJDTableViewButtonLingLiao) {
        ...
        guaDanVc.mOrderFrame = cell.orderFrame;
        guaDanVc.mOrderFrame.order.opened = NO;
        [mZgJobVc.navigationController pushViewController:guaDanVc animated:YES];
    } else if (button == XHDJDTableViewButtonCaozuo) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    if (self.DjdOrderFrames.count == self.totalCount) {
        [self.dzpTableView.mj_footer endRefreshingWithNoMoreData];
    }else{
        self.dzpTableView.mj_footer.state = MJRefreshStateIdle;
    }
...
- (XHOrderCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell.lingLiaoBtn setTitle:@"挂单" forState:UIControlStateNormal];
