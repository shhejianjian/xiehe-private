...
@interface XHMyJobViewController ()<UIScrollViewDelegate,CDRTranslucentSideBarDelegate,UITableViewDataSource, UITableViewDelegate>
...
@implementation XHMyJobViewController
...
- (void)setupChildVces
{
    XHDJDViewController *DJDVC = [[XHDJDViewController alloc] init];
    DJDVC.title = @"待接单";
    [self addChildViewController:DJDVC];
    
    XHYJDViewController *YJDVC = [[XHYJDViewController alloc] init];
    YJDVC.title = @"已接单";
    [self addChildViewController:YJDVC];
    
    XHYGQViewController *YGQVC = [[XHYGQViewController alloc] init];
    YGQVC.title = @"已挂起";
    [self addChildViewController:YGQVC];
    
    XHJDViewController *JDVC = [[XHJDViewController alloc] init];
    JDVC.title = @"已结单";
    [self addChildViewController:JDVC];
...
{
    // 监听待接单数量改变
    [XHNotificationCenter addObserver:self selector:@selector(DJDOrderCountDidChange:) name:XHDJDOrderCountDidChangeNotification object:nil];
    // 监听已接单数量改变
    [XHNotificationCenter addObserver:self selector:@selector(YJDOrderCountDidChange:) name:XHYJDOrderCountDidChangeNotification object:nil];
    // 监听已挂起数量改变
    [XHNotificationCenter addObserver:self selector:@selector(YGQOrderCountDidChange:) name:XHYGQOrderCountDidChangeNotification object:nil];
    // 监听已结单数量改变
    [XHNotificationCenter addObserver:self selector:@selector(JDOrderCountDidChange:) name:XHJDOrderCountDidChangeNotification object:nil];
}

- (void)DJDOrderCountDidChange:(NSNotification *)notification
{
    XYTitleButton *button = self.buttons[0];
    NSString *count = notification.userInfo[XHDJDOrderCount];
    button.countStr = count;
//        NSLog(@"待接单数量%@",notification.userInfo[XHDJDOrderCount]);
}
- (void)YJDOrderCountDidChange:(NSNotification *)notification
{
    XYTitleButton *button = self.buttons[1];
    button.countStr = notification.userInfo[XHYJDOrderCount];
//    NSLog(@"已接单数量%@",notification.userInfo[XHDJDOrderCount]);
}
- (void)YGQOrderCountDidChange:(NSNotification *)notification
{
    XYTitleButton *button = self.buttons[2];
    button.countStr = notification.userInfo[XHYGQOrderCount];
//    NSLog(@"已挂起数量%@",notification.userInfo[XHYGQOrderCount]);
}
- (void)JDOrderCountDidChange:(NSNotification *)notification
{
    XYTitleButton *button = self.buttons[3];
    button.countStr = notification.userInfo[XHJDOrderCount];
//    NSLog(@"结单数量%@",notification.userInfo[XHJDOrderCount]);
}
